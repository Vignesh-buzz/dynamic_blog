...
$sql="SELECT * FROM blog LEFT JOIN categories ON blog.category=categories.cat_id LEFT JOIN users ON blog.author_id=users.user_id WHERE cat_id='$id' ORDER BY blog.publish_date DESC";
				...
				<div class="card-body d-flex blog_flex">
					<div class="flex-part1">
						<a href="single_post.php?id=<?= $result['blog_id'] ?>">
                            <?php $img=$result['blog_image'] ?><img src="admin/upload/<?= $img ?>"> </a>
					</div>
					<div class="flex-grow-1 flex-part2">
						...
						<ul>
						<li>
							 <a href="single_post.php?id=<?= $result['author_id'] ?>" class="text-primary"> <span><i class="fa fa-tag" aria-hidden="true"></i></span> 
                              <?=$result['username'] ?>
                            </a>
						    </li>
							...
							<li>
								<a href="single_post.php?id=<?= $result['category'] ?>" class="text-primary"> <span><i class="fa fa-tag" aria-hidden="true"></i></span> 
                              <?=$result['cat_name'] ?>
                            </a>
						    </li>
						</ul>
